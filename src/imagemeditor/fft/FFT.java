/**
 * Uma explicação sobre o FFT pode ser encontrando em:
 * http://homepages.inf.ed.ac.uk/rbf/HIPR2/fourier.htm#4
 */

package imagemeditor.fft;
...
public final class FFT {
    ...
    void transform() {
        ...
        for (int i = 0; i < input.size; ++i) {
            progress++;
            campoTexto.setText("FFT em andamento (" + progress +")");
    ...
    private int[] toPixels(double[] values) {
        ...
        for (int i = 0; i < values.length; ++i) {;
            pixels[i] = (int) Math.round(values[i] * scale * 256);
        }
        return pixels;
    }
